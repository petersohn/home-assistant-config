...
class Enabler(hass.Hass):
    def _init_enabler(self, state):
        ...
        self.callbacks_mutex = self.get_app("locker").get_mutex("Enabler.Callbacks")
    ...
    def change(self, state):
        ...
        with self.state_mutex.lock("change"):
            ...
            self.change_timer = self.run_in(self.on_timeout, self.delay.total_seconds())
...
class EntityEnabler(Enabler):
    def do_initialize(self):
        ...
        self.listen_state(self._on_change, self._entity)
    ...
    def _on_change(self, entity, attribute, old, new, kwargs):
        with self.mutex.lock("_on_change"):
            value = self._get()
            self.log(
                "state change {}: {} -> {} value={}".format(entity, old, new, value)
            )
            self.change(value)
...
class ValueEnabler(EntityEnabler):
    def do_initialize(self):
        ...
        EntityEnabler.do_initialize(self)
...
class DateEnabler(Enabler):
    def do_initialize(self):
        self.begin = datetime.datetime.strptime(self.args["begin"], "%m-%d").date()
        self.end = datetime.datetime.strptime(self.args["end"], "%m-%d").date()
        self._init_enabler(self._get())
        self.run_daily(lambda _: self.change(self._get()), datetime.time(0, 0, 1))
...
class MultiEnabler(Enabler):
    def do_initialize(self):
        self.enablers = [self.get_app(enabler) for enabler in self.args.get("enablers")]
    ...
    def do_terminate(self):
        ...
        super(MultiEnabler, self).do_terminate()
...
class ExpressionEnabler(Enabler):
    ...
    def do_terminate(self):
        ...
        super(ExpressionEnabler, self).do_terminate()
