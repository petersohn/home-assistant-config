...
def _initialize(appdaemon_client, *configs):
...
def _start_control(appdaemon_client):
...
def _wait_for_control(history_watcher):
    history_watcher.wait_for_history(control_switch, "on", control_switch, "off")


def _start_control_and_wait(appdaemon_client, history_watcher):
    _start_control(appdaemon_client)
    _wait_for_control(history_watcher)


def test_control(appdaemon_client, history_watcher):
...
def test_reload_because_of_dependency(appdaemon_client, history_watcher):
...
def test_only_reload_changed_apps(appdaemon_client, history_watcher):
    _initialize(
        appdaemon_client,
        *base_configs,
